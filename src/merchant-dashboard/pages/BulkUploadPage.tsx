...
export default function BulkUploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!session?.access_token) {
        setStatus('error');
        setErrorMsg(t('bulkUpload.noSessionError'));
        return;
      }
      ...
      if (!res.ok || !json.ok) {
        setStatus('error');
        setErrorMsg(json.error ?? t('bulkUpload.uploadFailedError'));
        return;
      }
      ...
      setStatus('error');
      setErrorMsg(err instanceof Error ? err.message : t('bulkUpload.unexpectedError'));
    }
  };
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '2rem 1rem' }} dir={direction}>
      ...
      {/* Step 1 — Excel file */}
      <StepLabel number={1} text={t('bulkUpload.step1Label')} />
      <div
        style={dropZone}
        onClick={() => excelRef.current?.click()}
      >
        <input ref={excelRef} type="file" accept=".xlsx,.xls"
          style={{ display: 'none' }} onChange={handleExcelChange} aria-hidden="true" title={t('bulkUpload.excelInputTitle')} />
        {excelFile ? (
          <p style={{ margin: 0, color: '#4f2d91', fontWeight: 600 }}>
            {excelFile.name}{' '}
            <span style={{ color: '#888', fontWeight: 400 }}>
              {t('bulkUpload.fileSize', { size: (excelFile.size / 1024).toFixed(1) })}
            </span>
          </p>
        ) : (
          <>
            <UploadIcon />
            <p style={{ margin: '0.5rem 0 0', color: '#888' }}>{t('bulkUpload.clickToChooseExcel')}</p>
          </>
        )}
      </div>

      {/* Step 2 — Image files (optional) */}
      <StepLabel number={2} text={t('bulkUpload.step2Label')} />
      <div
        style={{ ...dropZone, marginBottom: '1.25rem' }}
        onClick={() => imagesRef.current?.click()}
      >
        <input ref={imagesRef} type="file" accept="image/*" multiple
          style={{ display: 'none' }} onChange={handleImagesChange} aria-hidden="true" title={t('bulkUpload.imagesInputTitle')} />
        {imageFiles.length > 0 ? (
          <div style={{ textAlign: 'right' }}>
            <p style={{ margin: '0 0 0.4rem', color: '#4f2d91', fontWeight: 600 }}>
              {t('bulkUpload.imagesSelectedCount', { count: imageFiles.length })}
            </p>
          ...
          <>
            ...
            <p style={{ margin: '0.5rem 0 0', color: '#888' }}>
              {t('bulkUpload.clickToChooseImages')}
            </p>
            <p style={{ margin: '0.2rem 0 0', color: '#aaa', fontSize: '0.78rem' }}>
              {t('bulkUpload.imageNameHint')}
            </p>
          </>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem' }}>
        <button onClick={handleUpload} disabled={!excelFile || status === 'uploading'}
          ...
          {status === 'uploading' ? t('bulkUpload.uploading') : t('bulkUpload.uploadBtn')}
        </button>
        {(excelFile || report) && (
          <button onClick={reset} style={secondaryBtn}>{t('bulkUpload.resetBtn')}</button>
        )}
      </div>
        ...
        <div style={{ marginTop: '1.5rem' }}>
          ...
          {report.failureDetails.length > 0 && (
            <div style={{ marginTop: '1rem' }}>
              <p style={{ fontWeight: 600, color: '#b45309', marginBottom: '0.5rem' }}>
                {t('bulkUpload.failureDetailsLabel', { count: report.failureDetails.length })}
              </p>
          ...
          {report._debug && (
            <div style={{ marginTop: '1rem', padding: '0.75rem', background: '#f1f5f9', borderRadius: 8, fontSize: '0.8rem' }}>
              <p style={{ fontWeight: 600, color: '#334155', marginBottom: '0.4rem' }}>{t('bulkUpload.savedImagesLabel')}</p>
              <ul style={{ margin: 0, padding: '0 1.25rem', color: '#475569' }}>
                {report._debug.map((d, i) => (
                  <li key={i} style={{ marginBottom: '0.2rem' }}>
                    <strong>{d.title}</strong>:{' '}
                    {d.image_urls && d.image_urls.length > 0
                      ? <span style={{ color: '#16a34a' }}>{t('bulkUpload.imageCount', { count: d.image_urls.length })}</span>
                      : <span style={{ color: '#dc2626' }}>{t('bulkUpload.noImages')}</span>}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div style={{ marginTop: '2rem', padding: '1rem', background: '#f8f9fa', borderRadius: 8, fontSize: '0.82rem', color: '#555' }}>
        <strong>{t('bulkUpload.formatGuideTitle')}</strong>
        <table style={{ marginTop: '0.5rem', borderCollapse: 'collapse', width: '100%', fontSize: '0.8rem' }}>
          <thead>
            <tr style={{ background: '#e2e8f0', textAlign: 'right' }}>
              <th style={th}>{t('bulkUpload.tableColStatus')}</th>
              <th style={th}>{t('bulkUpload.tableColCellValue')}</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td style={td}>{t('bulkUpload.rowDeviceImage')}</td>
              <td style={{ ...td, direction: 'ltr' }}>shirt.jpg</td>
            </tr>
            <tr style={{ background: '#f8fafc' }}>
              <td style={td}>{t('bulkUpload.rowMultipleImages')}</td>
              <td style={{ ...td, direction: 'ltr' }}>shirt.jpg, back.jpg</td>
            </tr>
            <tr>
              <td style={td}>{t('bulkUpload.rowExternalLink')}</td>
              <td style={{ ...td, direction: 'ltr' }}>https://example.com/img.jpg</td>
            </tr>
            <tr style={{ background: '#f8fafc' }}>
              <td style={td}>{t('bulkUpload.rowNoImage')}</td>
              <td style={td}>{t('bulkUpload.rowLeaveEmpty')}</td>
            </tr>
          </tbody>
        </table>
      </div>
