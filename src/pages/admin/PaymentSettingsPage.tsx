...
function getDefaultValues(definitions: SettingDef[]): SettingsValues {
  return Object.fromEntries(definitions.map(s => [s.key, s.defaultValue]));
}
...
const PaymentSettingsPage: React.FC<{ embedded?: boolean }> = ({ embedded = false }) => {
  ...
  const CATEGORIES: CategoryDef[] = useMemo(() => [
    { key: 'fees', label: t('paymentSettings.categories.fees'), icon: '₪' },
  ], [t]);
  ...
  const SETTINGS_DEFINITIONS: SettingDef[] = useMemo(() => [
    {
      key: 'platform_commission_rate',
      label: t('paymentSettings.settings.platform_commission_rate.label'),
      unit: t('paymentSettings.settings.platform_commission_rate.unit'),
      type: 'ratio',
      defaultValue: 0.10,
      min: 0,
      step: 0.01,
      explanation: t('paymentSettings.settings.platform_commission_rate.explanation'),
      category: 'fees',
    },
    {
      key: 'delivery_fee',
      label: t('paymentSettings.settings.delivery_fee.label'),
      unit: t('paymentSettings.settings.delivery_fee.unit'),
      type: 'currency',
      defaultValue: 25,
      min: 0,
      step: 1,
      explanation: t('paymentSettings.settings.delivery_fee.explanation'),
      category: 'fees',
    },
  ], [t]);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!token) { setLoadError(t('paymentSettings.sessionExpired')); return; }
      try {
        const res = await fetch(`${API_BASE}/api/payments/payouts/config`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const json = await res.json();
        if (!res.ok) { setLoadError(json.error ?? t('paymentSettings.loadError')); return; }
        ...
        setLoadError(t('paymentSettings.connectionError'));
      }
    })();
  ...
  const handleSave = async () => {
    const errs = validate(values, SETTINGS_DEFINITIONS, t);
    if (Object.keys(errs).length > 0) { setErrors(errs); return; }
    setSaveState('saving');
    setSaveMessage(null);
    try {
      const token = await getToken();
      if (!token) { setSaveState('error'); setSaveMessage({ type: 'error', text: t('paymentSettings.sessionExpired') }); return; }
      const patch: SettingsValues = {};
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error ?? t('logisticsSettings.saveFailedShort'));
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(248,250,252,0.95)', backdropFilter: 'blur(10px)', borderBottom: '1.5px solid #E2E8F0', padding: '0 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', gap: 16, direction }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 800, color: '#0F2B4E', lineHeight: 1.1 }}>{t('paymentSettings.title')}</div>
              <div style={{ fontSize: 11, color: '#94A3B8', lineHeight: 1.1 }}>{t('paymentSettings.subtitle')}</div>
            </div>
          </div>
