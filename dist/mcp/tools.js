import { z } from "zod";
export function registerTools(server, metaService) {
    // أداة: جلب جميع المنتجات
    server.tool("get_catalog_products", "Fetch a list of all products from Meta catalog using Supabase Auth", {
    ...
    // أداة: البحث عن منتج
    server.tool("search_products", "Search for a specific product by name using Supabase Auth", {
        query: z.string(),
    ...
    server.tool("add_to_marketplace", "Format a product for the unified marketplace using Supabase Auth", {
        productId: z.string(),
        sb_auth_token: z.string().describe("Supabase User JWT"),
