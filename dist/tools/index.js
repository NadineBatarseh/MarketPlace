...
export function registerTools(server, metaService) {
    ...
    server.tool("get_catalog_products", "Fetch a list of all products from Meta catalog", {}, async () => {
        const data = await metaService.getCatalogProducts();
        return { content: [{ type: "text", text: JSON.stringify(data) }] };
    });
    // Tool: Search Products
    server.tool("search_products", "Search for a specific product by name", { query: z.string() }, async ({ query }) => {
        const results = await metaService.searchProducts(query);
        return { content: [{ type: "text", text: JSON.stringify(results) }] };
    });
    // Tool: Import to Marketplace
    server.tool("add_to_marketplace", "Prepare and format a product for the unified local marketplace", {
        productId: z.string(),
