...
server.tool("get_catalog_products", "Fetch all products from Meta catalog", async (_params) => {
    try {
        const limit = 25;
...
server.tool("get_product", "Get details of a specific product by ID", async (params) => {
    try {
...
server.tool("search_products", "Search for products in the catalog", async (params) => {
    try {
...
server.tool("add_to_marketplace", "Add a Meta catalog product to your marketplace database", async (params) => {
    try {
...
server.tool("bulk_import", "Import multiple products from Meta catalog to your marketplace", async (params) => {
    try {
        ...
        if (category_filter) {
            products = products.filter((p) => p.category?.toLowerCase().includes(category_filter.toLowerCase()));
        }
        const importSummary = {
...
server.tool("sync_product", "Sync a product's data from Meta catalog (get latest updates)", async (params) => {
    try {
